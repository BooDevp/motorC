...
void main() {
    ...
    // 3. Efecto de Brasa (Naranja intenso con latido)
    float latido = 1.2 + 0.6 * sin(iTime * 4.0);
    vec3 colorBrasa = vec3(1.0, 0.25, 0.0) * latido * 2.5;

    // 4. Mezcla final: Donde mask es 1.0 (blanco) se ve la brasa
    vec3 finalRGB = mix(colorBase, colorBrasa, mask);
